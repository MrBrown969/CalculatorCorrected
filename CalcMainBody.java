...
public class CalcMainBody {
  ...
  static  int n2;
   ...
   static RomanCalc testR = new RomanCalc();
    static int number4;//эту переменную буду возвращать. Присвоил ей значения всех операций(в т.ч. значения операций до преобразования результата в римские цифры, превратил в String, вывел)
    
  static String calc (String fromLine){
    String input = fromLine;
 ...
 if(input.contains("+")&&(input.contains("V")||input.contains("I")||(input.contains("X")))){
    ...
    String number2 = plus[1];
     if(plus.length>2){                //добавил, чтобы было не более одной операции.Добавил такое же для "-","*","/"
       
       System.out.println("Ты ввел слишком много операций, можно только одну");
 ...
 if(plus.length>2){
   ...
   if(plus.length>2){
    ...
    }//в следующей строке немного условия изменил
    else if (testR.n3<=0&&(input.contains("V")||input.contains("I")||(input.contains("X")))&input.length()<9){
...
System.out.println("поэтому в ответе не может быть что-то меньше единицы.Помни об этом и у тебя всё получится!");
    ...
    String formalReturn = Integer.toString(number4);
